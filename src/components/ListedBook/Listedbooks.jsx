...
const Listedbooks = () => {
  ...
  useEffect(() => {
    ...
    const myreadlist = allBooks.filter((book) =>
      getReadListData.includes(book.bookId)
    );
  ...
  useEffect(() => {
    ...
    const mywishlistData = allBooks.filter((book) =>
      getwishlistdata.includes(book.bookId)
    );
  ...
  const handleSort = (criteria) => {
    ...
    if (criteria === "Rating") {
      setReadList((prev) => [...prev].sort((a, b) => b.rating - a.rating));
      setWishlist((prev) => [...prev].sort((a, b) => b.rating - a.rating));
    } else if (criteria === "Year") {
      setReadList((prev) => [...prev].sort((a, b) => b.yearOfPublishing - a.yearOfPublishing));
      setWishlist((prev) => [...prev].sort((a, b) => b.yearOfPublishing - a.yearOfPublishing));
    } else if (criteria === "Pages") {
      setReadList((prev) => [...prev].sort((a, b) => b.totalPages - a.totalPages));
      setWishlist((prev) => [...prev].sort((a, b) => b.totalPages - a.totalPages));
    }
  };
    ...
    <div className="px-4 md:px-8">
      ...
      <div className="mt-10 mb-10">
        <Tabs>
          ...
          <TabPanel>
            {readList.length === 0 ? (
              <p>Read List is empty. Please add some books.</p>
            ) : (
          ...
          <TabPanel>
            {wishlist.length === 0 ? (
              <p>Wishlist is empty. Please add some books.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
                {wishlist.map((book) => (
                  <div
